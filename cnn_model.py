...
def build_cnn_model_resnet(ccmodel, dmodel, ymodel):
    ...
    if CCMODEL:
        ...
        cc_model = Model(input_layer, x, name='modelCC')
        cc_model.compile(optimizer=opt, loss='binary_crossentropy', metrics=['accuracy'])
        cc_model.summary()
        return cc_model
        ...
        d_model = Model(input_layer, x, name='modelD')
        ...
        y_model = Model(input_layer, x, name='modelY')
...
def build_cnn_model_default(ccmodel, dmodel, ymodel):
    ...
    if CCMODEL:
        ...
        cc_model = Model(input, x, name='modelCC')
        cc_model.compile(optimizer=opt, loss='binary_crossentropy', metrics=['accuracy'])
        cc_model.summary()
        return cc_model
        ...
        d_model = Model(input, x, name='modelD')
        d_model.compile(optimizer=opt, loss='sparse_categorical_crossentropy', metrics=['accuracy'])
        d_model.summary()
        return d_model
        ...
        y_model = Model(input, x, name='modelY')
        y_model.compile(optimizer=opt, loss='sparse_categorical_crossentropy', metrics=['accuracy'])
        y_model.summary()
        return y_model
